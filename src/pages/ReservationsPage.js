...
const OCCASIONS = ['', 'Birthday', 'Anniversary', 'Date Night', 'Business Meal', 'Other'];
    ...
    return (
      <div className="res-confirmed">
        <div className="res-confirmed-inner">
          ...
          <p className="confirmed-sub">Thank you for choosing<br />Little Lemon.</p>
          ...
          <div className="confirmed-summary">
            ...
            <div className="confirmed-row">
              ...
              <span className="confirmed-row-right">{guests} {guests === 1 ? 'person' : 'people'}</span>
            </div>
          </div>

          <div className="confirmed-notice">
            <FontAwesomeIcon icon={faEnvelope} />
            <span>Confirmation sent to your email &amp; phone. Check your inbox or messages.</span>
          </div>

          <button className="res-btn-outline">
            <FontAwesomeIcon icon={faCalendarPlus} /> Add to Calendar
          </button>
          <Link to="/" className="res-btn-solid">Back To Home</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="res-page">
      ...
      <div className="res-progress">
        <div className={`res-step ${step >= 1 ? 'active' : ''}`}>
          <div className="res-step-circle">{step > 1 ? <FontAwesomeIcon icon={faCheck} size="xs" /> : '1'}</div>
          <span>Date &amp; Time</span>
        </div>
              ...
              <div className="res-slots-grid">
                {TIME_SLOTS.map((slot) => (
                  <button
                    ...
                    className={`res-slot${!slot.available ? ' unavailable' : ''}${time === slot.time ? ' selected' : ''}`}
      ...
      {step === 2 && (
        <form className="res-body" onSubmit={handleConfirm}>
          ...
          <div className="res-field-card res-summary-row">
            ...
            <button type="button" className="res-change-btn" onClick={() => setStep(1)}>
              Change
            </button>
          </div>
          ...
          <div className="res-field-card">
            ...
            <div className="res-stepper">
              ...
              >−</button>
              <span className="res-stepper-value">{guests}</span>
              <button
                type="button"
                className="res-stepper-btn"
          ...
          <p className="res-confirm-note">You'll receive instant email &amp; SMS confirmation</p>
        </form>
      )}
    </div>
  );
